...
impl MainMenu {
    pub fn new() -> Self {
        Self {
            ...
            height: 600,
            logo: None,
            logo_size: 150.0, // This is the target height of the logo
        }
    }
            ...
            ui.vertical_centered(|ui| {
                ...
                egui::Frame::group(ui.style())
                    ...
                    .show(ui, |ui| {
                        ui.set_width(350.0);
                        ui.vertical_centered(|ui| {
                            ui.heading(RichText::new(get_text("canvas_dimensions")).size(20.0));
                            ui.add_space(15.0);

                            ui.horizontal(|ui| {
                                ui.label(RichText::new(get_text("width")).size(16.0));
                                ui.add(egui::DragValue::new(&mut self.width).speed(1).clamp_range(100..=4000));
                            });

                            ui.horizontal(|ui| {
                                ui.label(RichText::new(get_text("height")).size(16.0));
                            ...
                            let button_text = RichText::new(get_text("create_new_canvas"))
                                .size(18.0)
                                .color(Color32::WHITE);
                            
                            if ui.add(egui::Button::new(button_text).min_size(Vec2::new(200.0, 36.0))).clicked() {
                                result = Some(MenuAction::NewCanvas(self.width, self.height));
                            }

                            ui.add_space(15.0);
                            
                            let open_file_text = RichText::new(get_text("open_file"))
                                .size(16.0);
                            
                            if ui.add(egui::Button::new(open_file_text).min_size(Vec2::new(180.0, 30.0))).clicked() {
                                result = Some(MenuAction::OpenFile);
                            }
                        });
                    });
